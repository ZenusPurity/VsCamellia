...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(_camsave.data.draintoggle == null){_camsave.data.draintoggle=false;}
		if(_camsave.data.vocals == null){_camsave.data.vocals=true;}
		if(_camsave.data.cambump == null){_camsave.data.cambump=true;}
		if(_camsave.data.characters == null){_camsave.data.characters=true;}
		if(_camsave.data.modcharts == null){_camsave.data.modcharts=false;}
		trace(_camsave.data);
	...
	function startIntro()
	{
		...
		camelliaIdle = new FlxSprite(FlxG.width * 0.35, FlxG.height * 0.01);
		camelliaIdle.frames = Paths.getSparrowAtlas('characters/camellia','shared');
		camelliaIdle.animation.addByPrefix('dance', 'Camellia_Idle', 24);
		camelliaIdle.flipX = true;
		camelliaIdle.scale.set(0.85,0.85);
		camelliaIdle.antialiasing = true;
	...
	override function beatHit()
	{
		...
		if (danceLeft)
			camelliaIdle.animation.play('dance');
		else
			camelliaIdle.animation.play('dance');
